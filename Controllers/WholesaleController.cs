...
public class WholesaleController : Controller
{
    ...
    public async Task<IActionResult> Apply(WholesaleApplyViewModel model)
    {
        ...
        return RedirectToAction(nameof(Confirmation), new { id = account.Id });
    }

    public async Task<IActionResult> Confirmation(int id)
    {
        var userId = _userManager.GetUserId(User)!;
        var account = await _context.WholesaleAccounts.FirstOrDefaultAsync(w => w.Id == id && w.UserId == userId);
        if (account == null)
        {
            return NotFound();
        }
