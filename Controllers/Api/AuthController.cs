...
public class AuthController : ControllerBase
{
    ...
    public AuthController(UserManager<ApplicationUser> userManager, ApplicationDbContext context, IJwtService jwtService, ILogger<AuthController> logger)
    {
    ...
    public async Task<IActionResult> Register(RegisterRequest request)
    {
        ...
        if (!result.Succeeded)
        {
            _logger.LogWarning("API registration failed for {Email}: {Errors}", request.Email,
                string.Join("; ", result.Errors.Select(e => e.Description)));
            return BadRequest(new { errors = result.Errors.Select(e => e.Description) });
        }

        await _userManager.AddToRoleAsync(user, "Customer");
        _logger.LogInformation("New account registered via API: user {UserId}.", user.Id);
    ...
    public async Task<IActionResult> Login(LoginRequest request)
    {
        ...
        {
            // Never log the submitted password or which factor failed.
            _logger.LogWarning("Failed API login attempt for {Email}.", request.Email);
            return Unauthorized(new { message = "Incorrect email or password." });
        }

        _logger.LogInformation("Successful API login for user {UserId}.", user.Id);
        return await IssueTokensAsync(user);
    }
