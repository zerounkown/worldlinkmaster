...
// (T-shirt family: brand, neck type, sleeve length, pattern; pants family: brand, pattern) — a
// stand-in for the real values that arrive via the Excel import, per the mega-menu sub-filtering
// request. Deliberately NOT wired into SeedData's automatic startup seed, since unlike
// AttributeDefinition rows (empty metadata, harmless), this writes real per-product data (BrandId
// reassignment + ProductAttributeValue rows) that would be visible to real shoppers. Only ever
// runs when explicitly invoked — see the /dev/mock-apparel-attrs/* endpoints in Program.cs
// (Development-only, behind a confirm=yes query flag).
//
// NECK-TYPE, SLEEVE-LENGTH, and PATTERN are the real Attribute Dictionary codes from the Master
// Data workbook (Areas/Admin/Controllers/MasterDataController.cs) — this script targets those
// directly rather than inventing its own, so mock and real data land in the same place. There is
// no separate "pants type" attribute in the real dictionary; PATTERN covers both families.
//
...
// non-filterable attribute — see SeedData.SeedInternalAttributeDefinitionsAsync) holding the exact
...
//
// Neck type / sleeve length are derived from the product name wherever it already spells the
// answer out (e.g. "Long Sleeve Crew Neck T-Shirt"); pattern has no such hint in any current
// product name, so it always uses the deterministic fallback. Never random, so re-running the
// seed always produces the same result for the same product. Values are strictly additive: a
// product that already has a value for one of these attributes (mock or real) is left untouched,
// never overwritten. That also means: run RemoveAsync before a real Excel import starts
// populating NECK-TYPE/SLEEVE-LENGTH/PATTERN — once real values exist under these same codes,
// remove can no longer tell them apart from leftover mock ones and should not be run.
public static class MockApparelAttributeSeeder
{
    ...
    private static readonly string[] PatternFallback = { "Solid", "Camouflage", "Multicam", "Digital" };

    public record SeedResult(int ProductsScanned, int BrandsReassigned, int NeckTypeValuesSet, int SleeveLengthValuesSet, int PatternValuesSet);
    ...
    public static async Task<SeedResult> SeedAsync(ApplicationDbContext db)
    {
        ...
        var attrDefs = await db.AttributeDefinitions
            .Where(a => a.Code == "NECK-TYPE" || a.Code == "SLEEVE-LENGTH" || a.Code == "PATTERN" || a.Code == BrandMarkerCode)
            .ToListAsync();
        var neckTypeDef = RequireDef(attrDefs, "NECK-TYPE");
        var sleeveLengthDef = RequireDef(attrDefs, "SLEEVE-LENGTH");
        var patternDef = RequireDef(attrDefs, "PATTERN");
        ...
        foreach (var product in products)
        {
            ...
            // Pattern applies to both families — a t-shirt or a pair of trousers can equally be
            // solid or camouflage-printed.
            var pattern = DeterministicPick(PatternFallback, product.Id);
            if (AddAttributeValueIfMissing(db, product, patternDef, pattern)) patternSet++;
        }

        await db.SaveChangesAsync();
        return new SeedResult(products.Count, brandsReassigned, neckSet, sleeveSet, patternSet);
    }
    ...
    private static AttributeDefinition RequireDef(List<AttributeDefinition> defs, string code) =>
        defs.FirstOrDefault(a => a.Code == code)
            ?? throw new InvalidOperationException($"{code} attribute definition not found — import the Master Data workbook's Attribute Dictionary sheet first.");
