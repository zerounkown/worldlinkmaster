...
// Shop mega-menu promo panel: hovering (or keyboard-focusing) a category column swaps the shared
// image/caption/CTA on the right to that category's data, read from data-promo-* attributes set
// server-side. The panel is rendered already populated with the first category so it's never
// empty on open — this only handles the swap on top of that.
(function () {
    var columns = document.querySelectorAll(".main-nav-mega-col");
    var promoImage = document.querySelector(".js-mega-promo-image");
    var promoCaption = document.querySelector(".js-mega-promo-caption");
    var promoCta = document.querySelector(".js-mega-promo-cta");
    var promoCtaText = document.querySelector(".js-mega-promo-cta-text");
    if (columns.length === 0 || !promoImage) {
        return;
    }

    function activate(col) {
        var image = col.getAttribute("data-promo-image");
        var caption = col.getAttribute("data-promo-caption");
        var ctaHref = col.getAttribute("data-promo-cta-href");
        var ctaText = col.getAttribute("data-promo-cta-text");
        if (image) {
            promoImage.setAttribute("src", image);
        }
        if (caption && promoCaption) {
            promoCaption.textContent = caption;
        }
        if (ctaHref && promoCta) {
            promoCta.setAttribute("href", ctaHref);
        }
        if (ctaText && promoCtaText) {
            promoCtaText.textContent = ctaText;
        }
    }

    columns.forEach(function (col) {
        col.addEventListener("mouseenter", function () { activate(col); });
        col.addEventListener("focusin", function () { activate(col); });
    });
})();
